...
namespace ricaun.Revit.ExtensibleJson.Converters
{
    ...
    public class ElementIdConverter : ObjectJsonConverter<ElementId>
    {
        ...
        /// <returns>A new <see cref="Autodesk.Revit.DB.ElementId"/> instance, or <c>null</c> if no suitable constructor is found.</returns>
        private static ElementId NewElementId(long id)
        {
            ...
            {
                var constructor = type.GetConstructor(new[] { constructorType });
                if (constructor != null)
                {
                    // Debug.WriteLine($"Constructor: {constructor}");
                    return constructor.Invoke(new object[] { Convert.ChangeType(id, constructorType) }) as ElementId;
                }
            }
            return null;
        }
        ...
        /// <returns>The value of the <see cref="Autodesk.Revit.DB.ElementId"/>, or 0 if not found.</returns>
        private static long GetValue(ElementId elementId)
        {
            ...
            foreach (var name in new[] { ElementIdKey, ElementIdKeyInteger })
            {
                ...
                if (property != null)
                {
                    ...
                    return (long)Convert.ChangeType(property.GetValue(elementId), typeof(long));
                }
            }
            return 0;
        }
        ...
        /// <returns>A <see cref="JObject"/> representing the <see cref="Autodesk.Revit.DB.ElementId"/>.</returns>
